...
printf 'RUN_ID=%s\nSTREAM_HEALTH=%s\nSTREAM_REASON=%s\nSTREAM_SOURCE=%s\nSTREAM_QUALITY_POLICY=%s\nSELECTED_STREAM=%s\nSTREAM_LAST_ERROR=%s\nSTREAM_LAST_RC=%s\nSTREAM_RETRY_IN=%s\nSTREAM_UPDATED_UTC=%s\n' "$RUN_ID" "$STREAM_HEALTH" "$STREAM_REASON" "$STREAM_SOURCE" "$STREAM_QUALITY_POLICY" "$SELECTED_STREAM" "$STREAM_LAST_ERROR" "$STREAM_LAST_RC" "$STREAM_RETRY_IN" "$STREAM_UPDATED_UTC"
